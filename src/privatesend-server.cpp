...
void CPrivateSendServer::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if (fLiteMode) return; // ignore all Voice related functionality
...
/*
    ...
    allow endless transaction that would bloat Voice and make it unusable. To
...
{
    if (fLiteMode) return;        // disable all Voice specific functionality
