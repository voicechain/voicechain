...
const char * const BITCOIN_CONF_FILENAME = "voice.conf";
const char * const BITCOIN_PID_FILENAME = "voiced.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL || *cacheCounter != logAcceptCategoryCacheCounter.load())
        {
            ...
            if (mapMultiArgs.count("-debug")) {
                ...
                // "voice" is a composite category enabling all Voice-related debug output
                if(ptrCategory->count(std::string("voice"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Voice
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Voice
    // Mac: ~/Library/Application Support/Voice
    // Unix: ~/.voice
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Voice";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Voice";
#else
    // Unix
    return pathRet / ".voice";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty voice.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override voice.conf
