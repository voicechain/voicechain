...
# voice_hash
RUN git clone https://github.com/voicepay/voice_hash
RUN cd voice_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} voice
RUN useradd -u ${USER_ID} -g voice -s /bin/bash -m -d /voice voice
...
RUN mkdir /voice-src && \
  ...
  chown $USER_ID:$GROUP_ID /voice-src && \
