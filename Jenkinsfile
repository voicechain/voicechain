...
// which are part of the Voice repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="voice-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /voice-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "voicepay-voice/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from voicepay-voice/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'voicepay-voice/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from voicepay-voice/develop"
            }
          }

          if (hasCache) {
            sh "cd /voice-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /voice-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /voice-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /voice-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /voice-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /voice-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /voice-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
